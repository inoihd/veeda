...
function Veeda(){
        ...
        (async()=>{
          try{
            ...
            if(redirectList.length===1){
              ...
              if(rSession?.profileId===rProfile.id&&rSession?.pw){
                await doEnterProfile(rProfile,rSession.pw);
              }else{
                setScreen("password");
              }
            }else{
  ...
  // ── Entrada no app com perfil + senha (comum a todos os fluxos de login) ────
    ...
    if(profile.cloud){
      ...
      if(t){
        try{
          ...
          if(remote&&!safeLS.raw(`veeda_data_${profile.id}`)){
            safeLS.rawSet(`veeda_data_${profile.id}`,remote.content);
          }
  ...
  const doGoogleLogin=useCallback(async()=>{
    ...
    try{
      ...
      if(list.length===1){
        ...
        // Se há sessão salva para este perfil, vai direto pro app sem pedir senha
        if(session?.profileId===profile.id&&session?.pw){
          await doEnterProfile(profile,session.pw);
        }else{
          setScreen("password");
        }
      }else{
  ...
  if(screen==="google_select")return<ScreenGoogleSelect
    ...
    hasLocal={hasLocal}
    onSelect={async p=>{const sp={...p,cloud:true};setSelProfile(sp);const ss=loadSession();if(ss?.profileId===sp.id&&ss?.pw){await doEnterProfile(sp,ss.pw);}else{setScreen("password");}}}
