...
function PollResultBar({ option, votes, total }) {
...
export default function Polls() {
  ...
  const loadVoteCounts = useCallback(async (pollId) => {
  ...
  // Realtime subscription on poll_votes
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'poll_votes' }, (payload) => {
        ...
        if (pollId) loadVoteCounts(pollId)
      })
      .subscribe()
  ...
  async function handleDelete(poll) {
    ...
    await logAction('delete_poll', { poll_id: poll.id, question: poll.question })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{polls.length} enquetes — resultados em tempo real</p>
        </div>
          ...
          {polls.map(poll => {
            ...
            return (
              <div key={poll.id} className="card">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {poll.poll_options?.length ?? 0} opções
                      {isExpanded && ` · ${totalVotes} votos`}
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    <button onClick={() => toggleExpand(poll.id)} className="btn-secondary text-xs">
                      {isExpanded ? 'Ocultar' : 'Resultados'}
                    </button>
                    <button onClick={() => toggleActive(poll)} className="btn-secondary text-xs">
                ...
                {isExpanded && (poll.poll_options ?? []).length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Resultados — {totalVotes} votos</p>
